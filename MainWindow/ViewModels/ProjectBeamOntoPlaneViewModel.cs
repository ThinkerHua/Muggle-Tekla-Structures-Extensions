...
namespace Muggle.TsExtensions.MainWindow.ViewModels {
    ...
    public partial class ProjectBeamOntoPlaneViewModel : ViewModelBase {
        ...
        [ObservableProperty]
        private ProjectionDirectionEnum _projectionDirection;

        [ObservableProperty]
        private WayToGetPlaneEnum _wayToGetPlane;
        ...
        private void ProjectBeamOntoPlane() {
            ...
            var plane = new GeometricPlane(points[0], new Vector(points[1] - points[0]), new Vector(points[2] - points[0]));

            var direction = ProjectionDirection switch {
                ProjectionDirectionEnum.Perpendicular => plane.Normal,
                _ => new Vector(0, 0, 1000).TransformFrom(new TransformationPlane()),
            };
