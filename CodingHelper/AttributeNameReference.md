...
- The string "**\<nameOrNumber>**" represents the actual name or number setted, including the angle brackets.

- "**Plate**" is not an actual model object type in OpenAPI, the way to create it depends on the programmer.

- "BoltGroup.**BoltDistX**" and "BoltGroup.**BoltDistY**" are not actual properties in OpenAPI, they map to "**AddBoltDistX**" and "**AddBoltDistY**" methods of "BoltArray" or "BoltXYList".
...
| Part.Assembly.Prefix | PT\<nameOrNumber>ASMP
| Part.Assembly.StartNumber | PT\<nameOrNumber>ASMN
...
| Plate.Assembly.Prefix | PL\<nameOrNumber>ASMP
| Plate.Assembly.StartNumber | PL\<nameOrNumber>ASMN
