...
namespace Muggle.TsExtensions.CodingHelper.Generators {
    ...
    internal class PluginDataFieldsGenerator : IIncrementalGenerator {
        ...
        private static PluginDataFieldsInfo Transform(GeneratorSyntaxContext syntaxContext, CancellationToken token) {
        ...
        private static void Generate(SourceProductionContext context, PluginDataFieldsInfo fieldsInfo) {
            ...
            foreach (var kvp in fieldsInfo.Arguments) {
                var attName = kvp.Key;
                var attPrefix = attName.Length > 15 ? attName.Substring(0, kvp.Key.Length - 15) : string.Empty;
            ...
            foreach (var nameOrNumber in kvp.Value) {
                foreach (var info in infos) {
                    fieldsBuilder.AppendLine(
                        $"        \n" +
                        $"        [global::Tekla.Structures.Plugins.StructuresField(\"{attPrefix}{nameOrNumber}{info.AttributeName}\")]\n" +
                        $"        public {info.Type} {fieldPrefix}{nameOrNumber}{info.Name};");
                }
            }
        ...
        private static string GenerateGeneralFields(KeyValuePair<string, NameOrNumberSet> kvp) {
            ...
            foreach (var name in kvp.Value) {
                fieldsBuilder.AppendLine(
                    $"        \n" +
                    $"        [global::Tekla.Structures.Plugins.StructuresField(\"{name}\")]\n" +
                    $"        public {dataType} {ToPropertyNameStyle(name)};");
            }
